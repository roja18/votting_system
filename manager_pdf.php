...
{
    //Logo
    // $this->Image('image/user.GIF',5,5,35,35);
	 //Logo
...
function Footer()
{
  ...
  $this->SetFont('Arial','I',7);
	...
	$this->SetFont('Arial','I',7);
	$this->Cell(-30,10, 'created by Bogohe',0,0,'C');
}
}
...
$pdf->SetY($y);
$pdf->SetX($x);
$pdf->Cell(200, 5, 'LIST OF ELECTION MANAGER REGISTERD', 0, 0, 'C');
$y = $y + 7;
...
$pdf->SetY($y);
$pdf->SetX($x);
$pdf->Cell(35, 5, 'MANAGER NAME', 1, 0, 'C');

$x = $x + 35;
$pdf->SetX($x);
$pdf->Cell(45, 5, 'DATE OF BIRTH ', 1, 0, 'C');

$x = $x + 45;
$pdf->SetX($x);
$pdf->Cell(40, 5, 'ELECTION PLACE ', 1, 0, 'C');

$x = $x + 40;
$pdf->SetX($x);
$pdf->Cell(60, 5, 'EMAIL ADDLESS ', 1, 0, 'C');
...
$st = "SELECT * FROM instution WHERE typez='manager'";
...
// //     $pdf->Cell(35, 5,  1, 0, 'L');
    ...
    $pdf->SetY($y);
    $pdf->SetX($x);
    $pdf->Cell(45, 5, $dob, 1, 0, 'L');

    // $x = $x + 45;
    // $pdf->SetY($y);
    // $pdf->SetX($x);
    // $pdf->Cell(40, 20, "<img src='imagez/$img'>", 1, 0, 'L');

    $x = $x + 45;
    $pdf->SetY($y);
    $pdf->SetX($x);
    $pdf->Cell(40, 5, $place, 1, 0, 'L');
